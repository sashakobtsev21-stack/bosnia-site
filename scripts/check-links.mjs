 ...
 * сюда НЕ добавляем (внешние ссылки по-прежнему пропускаются). На одноязычном
 * сайте (только en на корне) языкового дерева нет — проверка пройдёт вхолостую.
 */
...
async function main() {
  ...
  console.log(
    `[test:links] OK — битых внутренних ссылок нет, /go/-ссылки с rel+?c=, языковые префиксы /ru/ и /uk/ на месте${langPrefixBaselined.length ? ` (${langPrefixBaselined.length} legacy в бейслайне)` : ''}.`,
  );
  process.exit(0);
}
