...
/**
 * Контент-модель Bosnia and Herzegovina Guidebook (SPEC §11, v3.1).
 *
 ...
 * Опционально, на контракт прочих категорий не влияет (§23). Слаги ↔ en
...
const ATTRACTION_TYPES = [
  ...
  'caves', // пещеры / каньоны рек
  'religious-sites', // мечети / церкви / монастыри / текке
  'fortresses-castles', // крепости / старые города
  'resorts-springs', // курорты / термы / горнолыжные
  'museums-landmarks', // музеи / городские объекты / военная память
] as const;

/**
 * Регион Боснии и Герцеговины (§7) — для фильтра каталога достопримечательностей.
 * Опционально (используют только достопримечательности). Слаги ↔ en лейблы в
 * i18n (`regions`). Не 142 общины, а 3 турлогических макрорегиона (план сайта
 * 2026-06-30): Герцеговина (юг/средиземноморье), регион Сараево (+ олимпийские
 * горы), север Боснии (Una/Jajce/Sutjeska/Banja Luka). Порядок = порядок опций.
 */
const REGIONS = [
  'herzegovina',
  'sarajevo-region',
 ...
 * для живущих в Боснии и Герцеговине (риелторы, клининг и т.п.). Слаги ↔ en
 * лейблы в i18n (`serviceRubrics`).
 */
...
const articleBase = z.object({
  ...
  /**
   * Регион Боснии и Герцеговины (§7) — фильтр/чип каталога достопримечательностей.
   ...
   * местах получают блок AccessFrom с расстоянием/временем из Сараево,
   * Мостара и Дубровника (хорватский хаб day-trip-трафика). Все поля внутри
   * опциональны — указываем только то, что известно (CLAUDE правило 4). На
   * контракт API (§23) не влияет.
   */
  accessFrom: z
    .object({
      sarajevo: z
 ...
 * живущих в Боснии и Герцеговине (риелторы, клининг, переезд и т.п.). Платное размещение —
