 ...
 */
export const EDA_CITY_KEYS = ['sarajevo', 'mostar', 'trebinje'] as const;
export type EdaCityKey = (typeof EDA_CITY_KEYS)[number];

/**
 * Слаги регионов Боснии и Герцеговины (§7) — совпадают с enum REGIONS в
 * content.config.ts (поле `region`). Лейблы — в словарях (`regions`).
 * 3 турлогических макрорегиона (план сайта 2026-06-30).
 */
export const REGION_SLUGS = ['herzegovina', 'sarajevo-region', 'northern-bosnia'] as const;
...
export interface UIDictionary {
  ...
  /** Блок «Сейчас в Боснии» на главной (§8.4): погода + курс марки BAM/KM. */
  liveData: {
    heading: string;
    ...
    weather: string;
    fx: string;
    cities: { sarajevo: string; mostar: string; banjaluka: string };
    updated: string;
    source: string;
  };
   ...
   * точек въезда. Лейблы городов-истоков по слугам sarajevo/mostar/dubrovnik.
   */
  access: {
   ...
   * InsuranceHub). Регуляторная тема: действующие правила въезда в Боснию и Герцеговину +
  ...
  /** Лейблы регионов Боснии и Герцеговины (§7) по слугам REGION_SLUGS. */
   ...
   * Боснии и Герцеговине. Шаблон-директория (ServicesDirectory) с фильтром по рубрике;
